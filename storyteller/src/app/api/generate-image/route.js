...
} from "@/lib/services/openaiImageService";
  ...
  try {
    ...
    if (!biblicalEvent) {
      return NextResponse.json(
        { error: "Missing scene information" },
        { status: 400 }
      );
    }
    ...
    const useMockImages = bypassMock
      ? false
      : envDisablesMock
        ? false
    ...
    console.log(
      `Image generation mode: ${useMockImages ? "MOCK" : "REAL"} (env: ${envMockValue}, config: ${config.imageGeneration.useMockImages}, bypass: ${bypassMock})`
    );
    ...
    if (useMockImages) {
      const placeholderIndex =
        Math.floor(
          Math.random() * (config.imageGeneration.mockImageCount || 5)
        ) + 1;
    ...
    if (apiRequestTracker.isOverLimit(5) && !forceRefresh) {
      ...
      const placeholderIndex =
        Math.floor(Math.random() * config.imageGeneration.mockImageCount) + 1;
      const placeholderDir =
        config.imageGeneration.mockImageDir || "/images/placeholders";
    ...
    try {
      ...
      imageUrl = await generateDalleImage(
        prompt,
        config.imageGeneration.imageSize,
        scene,
        language
      );
    } catch (error) {
