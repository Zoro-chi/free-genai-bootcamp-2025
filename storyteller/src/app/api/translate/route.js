...
export async function POST(request) {
  try {
    ...
    if (
      (actualProvider === "bedrock" || actualProvider === "amazonbedrock") &&
      !bedrockService.checkBedrockConfig()
    ) {
      console.warn(
        "AWS not properly configured, falling back to mock translation"
      );
      actualProvider = "mock";
    }
    ...
    console.log(
      `Translation request: ${actualProvider} for ${language}, text length: ${text.length}`
    );
      ...
      case "amazonbedrock":
        try {
          const translationPrompt =
            prompt ||
            bedrockService.createTranslationPrompt(
              text,
              language,
              sourceLanguage
            );
          ...
          if (bedrockClient) {
            translation = await bedrockService.translateWithBedrockModels(
              bedrockClient,
              translationPrompt,
              text
            );
