...
import {
  ...
  getImageForVerse,
  clearImageCache,
} from "../lib/services/chapterImageService";
import BibleImageLoader from "./BibleImageLoader";
import { translateVerses } from "../lib/services/translationService";
...
const BibleChapterViewer = ({
  ...
  const [viewMode, setViewMode] = useState("split"); // Only used for mobile now
  ...
  // Remove swipe-related state variables
  ...
  useEffect(() => {
    if (isMobile) {
      ...
      // Always use split view on desktop
      setViewMode("split");
    }
  }, [isMobile]);
  ...
  // Remove the swipe gesture setup effect

  // Remove the handleSwipe function

  // Navigation handlers
  const handlePrevChapter = () => {
    if (parseInt(chapter) > 1) {
      router.push(
        `/read?book=${book}&chapter=${parseInt(chapter) - 1}&lang=${language}`
      );
    }
  };
    ...
    const handleIntersect = (entries) => {
      ...
      if (nowVisible.length > 0) {
        ...
        if (centermostVerse && centermostVerse !== currentVerse) {
          console.log(
            `Setting current verse to ${centermostVerse} based on scroll`
          );
      ...
      try {
        ...
        try {
          ...
          const response = await axios.get(
            `/api/bible-content?book=${book}&chapter=${chapter}&language=${language}`
          );
            ...
            if (
              ...
              try {
                ...
                const trackImageProgress = (segments) => {
                  ...
                  const loadedCount = segments.filter(
                    (s) => s && !s.loading
                  ).length;
                ...
                const images = await generateChapterImages(
                  book,
                  chapter,
                  response.data.verses,
                  imageProvider,
        ...
        try {
          ...
          const bookData = kjvData.books.find(
          ...
          console.log(
            `Found book data for ${book} with ${bookData.chapters.length} chapters`
          );

          // Find the correct chapter in the book
          if (bookData.chapters.length < chapter) {
            console.error(
              `Chapter ${chapter} not found in ${book}, max is ${bookData.chapters.length}`
            );
            setLoading(false);
            return;
          }
          ...
          if (!chapterData) {
            console.error(
              `Chapter ${chapter} data could not be retrieved for ${book}`
            );
            setLoading(false);
            return;
          }
          ...
          const keyScenes = extractKeyScenes(
            bookData.name,
            parseInt(chapter),
            chapterData.verses
          );
  ...
  useEffect(() => {
    if (chapterImages && chapterImages.length > 0) {
      ...
      console.log(
        `Available images:`,
        chapterImages.map((img) => `Verses ${img.startVerse}-${img.endVerse}`)
      );
      console.log(
        `Current image:`,
        currentImage
      ...
      console.log(
        `Matching image:`,
        matchingImage
          ? `Verses ${matchingImage.startVerse}-${matchingImage.endVerse}`
          : "None"
      );
      ...
      if (matchingImage) {
        ...
        console.log(
          `Updated image for verse ${currentVerse}: verses ${matchingImage.startVerse}-${matchingImage.endVerse}`
        );
      }
    }
  ...
  const extractSetting = (verses) => {
    ...
    const foundPlaces = commonPlaces.filter((place) => text.includes(place));
  ...
  const containerStyle = {
    ...
    flexDirection: isMobile ? "column" : "row", // Always row for desktop
    ...
    marginTop: "2rem",
    // Prevent horizontal scrolling on mobile while allowing sticky to work
    maxWidth: isMobile ? "100%" : "1200px",
    // Remove overflow: hidden which can interfere with sticky positioning
  };

  const textColumnStyle = {
    flex: isMobile ? "1 0 auto" : "0 0 60%",
    ...
    marginTop: isMobile
      ? isImageCollapsed
        ? "90px"
        : "350px" // Dynamic top margin based on image state
      : 0,
    // Ensure text content doesn't overflow horizontally
    overflowWrap: "break-word",
    wordWrap: "break-word",
  };

  const imageColumnStyle = {
    flex: isMobile ? "1 0 auto" : "0 0 40%",
    ...
    maxHeight: isMobile
      ? isImageCollapsed
        ? "80px"
        : "calc(100vh - 120px)" // Better calculation for mobile
      : "calc(100vh - 60px)",
    transition: "max-height 0.3s ease-in-out",
    // Change overflow behavior for visible image but hidden horizontal scroll
    overflowY: "visible",
    overflowX: "hidden",
  ...
  return (
    <div className="bible-chapter-viewer max-w-6xl mx-auto">
      {loading ? (
        ...
        <div
          className="translating flex justify-center p-12 w-full"
        ...
        <>
          ...
          {/* {isMobile && (
            <div className="fixed bottom-4 right-4 z-50">
              <button
                ...
                aria-label={
                  showMobileControls ? "Hide navigation" : "Show navigation"
                }
              >
                {showMobileControls ? (
          ...
          <div
            className={`chapter-header mb-6 bg-bible-paper bg-opacity-80 p-${isMobile ? "3" : "6"} rounded-lg shadow-md border border-bible-scroll`}
          >
            <div className="flex justify-between items-center">
              {/* Add bookmarking UI - Remove view toggle buttons for desktop */}
              <div className="flex items-center gap-2">
                {/* Remove the desktop view toggle UI */}
            ...
            {/* Updated Bible Navigation with language handling and verses */}
            <div className="mt-6">
              <BibleNavigation
                currentBook={book}
                ...
                language={language}
                isMobile={isMobile}
                verses={content?.verses || []} // Pass verses to BibleNavigation
                onLanguageChange={(newLanguage) =>
                  router.push(
                    `/read?book=${book}&chapter=${chapter}&lang=${newLanguage}`
                  )
                }
              />
            </div>
            ...
            {selectedVerses.length > 0 && (
              <div
                className={`mt-4 ${isMobile ? "flex-col" : "flex justify-between"} items-${isMobile ? "start" : "center"}`}
              >
                <div className="text-bible-royal">
                  <span className="font-biblical">
          ...
          <div style={containerStyle}>
            ...
            <div style={imageColumnStyle}>
              {loadingImages ? (
                <BibleImageLoader
                  ...
                  chapter={chapter}
                  totalSegments={
                    config.imageGeneration.maxImagesPerChapter || 2
                  } // Use config value
              ...
              {/* Information about the current scene - commented out as requested */}
              {/*
              {currentImage && !loadingImages && (
                <div
                  className={`p-${isMobile ? "2" : "4"} border-t border-bible-scroll bg-bible-parchment`}
                >
                  <div
                    className={`mt-${isMobile ? "1" : "2"} ${isMobile ? "text-xs" : "text-sm"} text-bible-ink font-biblical`}
                  >
                    <div>
                      <span className="font-medium text-bible-royal">
                        Characters:
                      </span>{" "}
                    ...
                    <div>
                      ...
                      {currentImage.setting}
                    </div>
                    <div
                      className={`${isMobile ? "text-2xs" : "text-xs"} mt-${isMobile ? "1" : "3"} text-bible-ink opacity-70`}
                    >
                      Verses {currentImage.startVerse}-{currentImage.endVerse}
                    </div>
                  </div>
              ...
              */}
              ...
              {/* Remove mobile swipe helper text */}
            </div>
            ...
            {/* {process.env.NODE_ENV === "development" && (
              <div className="mb-4 p-2 bg-gray-100 border rounded-md">
                ...
                <div className="mt-2 text-xs text-gray-500">
                  <p>
                    Config limit:{" "}
                    {config.imageGeneration.maxImagesPerChapter || 2} images per
                    chapter
                  </p>
                  <p>
                    Env mock setting:{" "}
                    {process.env.NEXT_PUBLIC_USE_MOCK_IMAGES || "(not set)"}
                  </p>
                </div>
              </div>
            )} */}
            ...
            <div style={textColumnStyle} className="bible-text">
              <div className="bible-paper p-4 md:p-6">
                <h3 className="text-xl font-semibold mb-6 text-center font-biblical border-b pb-3 text-bible-royal">
                  Scripture Text
                </h3>
                <div className="space-y-4 font-biblical text-lg leading-relaxed px-2 md:px-6">
                  {content.verses.map((verse) => (
                    <div
                      key={verse.number}
                      className={`verse transition-colors duration-200 p-3 ${
                        currentVerse === verse.number
                          ? "bg-bible-scroll bg-opacity-20 rounded-md p-3"
                          : ""
                    ...
                    >
                      <div className="flex items-start">
                        ...
                        <div className="flex-grow">
                          <span
                            className={`verse-number ${isMobile ? "font-bold text-bible-gold" : ""}`}
                          >
                            {verse.number}
                          </span>
          ...
          {showExplainer && content && (
            <VerseExplainer
              verses={selectedVerses.map((num) =>
                content.verses.find((v) => v.number === num)
              )}
              book={book}
              chapter={chapter}
              language={language}
