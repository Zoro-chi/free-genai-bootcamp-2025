...
async function saveImageLocally(imageUrl, imageName) {
  ...
  try {
    ...
    const imageDir = path.join(
      publicDir,
      config.imageGeneration.localImageDir || "generated-images"
    );
...
export async function generateDalleImage(
  ...
  try {
    ...
    if (response && response.data && response.data[0] && response.data[0].url) {
      ...
      if (
        config.imageGeneration.saveLocally &&
        !process.env.DISABLE_IMAGE_SAVING
      ) {
        try {
          ...
          console.error(
            "Failed to save image locally, using original URL:",
            saveError
          );
          return imageUrl;
        }
    ...
    if (error.status === 429) {
      ...
      const waitTime = resetTimeHeader
        ? parseInt(resetTimeHeader) * 1000
        : 60000;
...
export function createBiblicalImagePrompt(scene, language = "english") {
  ...
  const culturalStyles = {
    ...
    yoruba: {
      ...
      people:
        "Yoruba people in traditional Nigerian attire like agbada, aso-oke, or gele headwear",
      setting: "setting adapted to Yoruba cultural context",
    },
    igbo: {
      style: "Nigerian Igbo cultural aesthetic",
      people:
        "Igbo people in traditional Nigerian attire like isiagu shirts, george wrapper, or ichafu caps",
